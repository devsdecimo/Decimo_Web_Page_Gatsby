...
const OpenPositions = (props) => {
  ...
  const title_section = {
    __html:
      useStaticQuery(query).allNodeOpenPositionsSection.nodes[0]
        .field_title_section.value,
  };
  const descripcion_section = {
    __html:
      useStaticQuery(query).allNodeOpenPositionsSection.nodes[0]
      ...
      <div className="background-gradient-color">
        ...
        <div className="openpositions-container">
          <div className="section-1">
            <a href={cardData[1].field_site_link.uri}>
              <div className="card-section">
                {/* Imagen de las cards */}
                <GatsbyImage
                  image={getImage(
                    cardData[1].relationships.field_image_card.localFile
                  )}
                ...
                <div className="job-slogan-section">
                  <h3 className="job-area">
                    {cardData[1].field_job_area}{" "}
                    <span className="text-separation"></span>
                  </h3>
                  <span className="slogan">{cardData[1].field_slogan}</span>
                </div>
                {/* Descripcion de las cards */}
                <hr className="div-separation"></hr>
                <div>
                  <p className="description-section">
                    {cardData[1].field_description_card}
                  </p>
                </div>
              </div>
            </a>
            <a href={cardData[0].field_site_link.uri}>
              <div className="card-section">
                {/* Imagen de las cards */}
                <GatsbyImage
                  image={getImage(
                    cardData[0].relationships.field_image_card.localFile
                  )}
                ...
                <div className="job-slogan-section">
                  <h3 className="job-area">
                    {cardData[0].field_job_area}{" "}
                    <span className="text-separation"></span>
                  </h3>
                  <span className="slogan">{cardData[0].field_slogan}</span>
                </div>
                {/* Descripcion de las cards */}
                <hr className="div-separation"></hr>
                <div>
                  <p className="description-section">
                    {cardData[0].field_description_card}
                  </p>
                </div>
              </div>
            </a>
...
const Wrapper = styled.div`
  ...
  span {
    background: linear-gradient(
      89.63deg,
      rgb(51, 153, 153, 1) 5.4%,
      rgba(255, 153, 51) 100.53%
    );
  ...
  .line-color {
    background: linear-gradient(
      89.63deg,
      rgb(51, 153, 153) 10.13%,
      rgb(255, 153, 51) 90.06%
    );
    padding: 3px;
    border: none;
    margin: 0px;
